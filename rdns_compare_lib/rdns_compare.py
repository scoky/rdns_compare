...
try:
    ...
    # Nothing we can do without this lib...
...
class Recursive(object):
    ...
    def _compute(self, times):
        ...
        if len(times) > 0:
            ...
            sd = self._sd(times, mean)

            tail = [v for v in times if v > mean + sd]
            if len(tail) > 0:
                sd_tail_mean = self._mean(tail)
                sd_tail_median = self._median(tail)

            median = self._median(times)
            md = self._median([abs(v - median) for v in times])
            tail = [v for v in times if v > median + md]
            if len(tail) > 0:
                md_tail_mean = self._mean(tail)
                md_tail_median = self._median(tail)
